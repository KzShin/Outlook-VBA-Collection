...
* **UTF-8バイナリ追記**: 日本語を含むログをUTF-8で保存します。ファイル全体を再読み込みせず末尾へ直接バイナリ追記（O(1)）するため、ログが肥大化しても動作が重くならず、クラッシュ時のファイル破損・データ消失リスクを防止します。
...
* **共通設定モジュール**: `modConfig.bas` (同梱の設定読み込みモジュール)
...
3. **モジュールのインポート**:
    * `File` > `Import File` から以下の**2つのファイル**をインポートします。
        1. `src/modConfig.bas` (共通設定モジュール)
        2. `src/modLogger.bas` (共通ログモジュール)
